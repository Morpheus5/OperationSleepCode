...
for i = 1:files_count
    ...
    roi_trace = NP_ExtractROIs(video.frames((1 + skip_frames):(end - skip_frames)), ROI, 1);
    ...
    roi_trace = NP_ExtractROIs(video.frames((1 + skip_frames):(end - skip_frames)), ROI, 0);
...
for i = 1:files_count
    ...
    for j = find(rois_of_interest)
        ...
        for k = find(cur_spikes(j, :))
            ...
            % get relevant frames
            frames = cat(3, video.frames((skip_frames + k - window(1)):(skip_frames + k + window(2))).cdata);
            ...
            if save_plot
                f = figure;
                plot_many(((skip_frames + k - window(1)):(skip_frames + k + window(2))) ./ fs, traces');
            ...
            if save_video
                ...
                d = false(size(spikes));
                d(j, :) = spikes(j, :);
                dff = NP_DrawROIs(dff, ROI, d, fs);
                
                % write
                video_write(fullfile(directory_out, num2str(j), sprintf('%s_%d.mp4', name, k)), dff, fs);
            end
